...
(function () {
  ...
  // From: http://stackoverflow.com/questions/5185326
  ...
  AuthenticatedRawRequest = function (auth, options) {
    ...
    // Unicode: Need `encodeURIComponent` to handle utf8 characters, which are
    // valid for AWS, etc. key names.
    if (auth.isGoogle()) {
      // Unfortunately, Google doesn't work with this.
      // Throw error if non-ascii characters.
      if (!isAscii(urlObj.pathname)) {
        self._err = new Error(
          "Google requires ascii path: " + urlObj.pathname);
      }
    } else {
      // Encode for UTF8 support.
      urlObj.pathname = encodeURIComponent(urlObj.pathname);
    }
      ...
      response.on('end', function () {
        ...
        if (self._buf.length > 0) {
          if (self._encoding) {
            ...
            data = self._buf
              .map(function (buf) { return buf.toString(self._encoding); })
              .join("");
          } else {
